...
(function () {
  ...
  function render(center) {
    const statusHtml = center.status === "verified"
    ...
    if (progWrap && Array.isArray(center.programs)) {
      progWrap.innerHTML = center.programs
        .map(
          (p) => `<div class="program-row"><div><div class="program-row__name">${p.name}</div><div class="program-row__meta">${p.format}</div></div><div class="program-row__meta">${p.duration}</div></div>`
        )
        .join("");
    }
    ...
    if (revWrap) {
      revWrap.innerHTML = reviews
        .map(
          (r) => `<div class="review"><div class="review__head"><span class="img-ph img-ph--avatar" style="width:40px;height:40px" aria-hidden="true"></span><div><div class="review__name">${r.name}</div><div class="review__stars">${"★".repeat(r.stars)}${"☆".repeat(5 - r.stars)}</div></div></div><p>${r.text}</p></div>`
        )
        .join("");
    }
  }
